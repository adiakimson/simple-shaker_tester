...
class ArduinoLogger:
    ...
    def start_logging(self):
        ...
        try:
            ...
            self.log_file = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv")])
            ...
            while self.logging:
                try:
                    if self.serial_port.in_waiting > 0:
                        ...
                        # Format the line to CSV format
                        values = line.replace("X:", "").replace("Y:", "").replace("Z:", "").split("\t")
                        csv_line = ",".join([v.strip() for v in values])
                        file.write(csv_line + '\n')
